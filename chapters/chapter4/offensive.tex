...
Although Lehmann and colleagues \cite{Hilbig2021AnES} observe a decline in browser-based cryptominers, mainly due to the shutdown of Coinhive, a 2022 report by Kaspersky indicates that the use of cryptominers is on the rise \cite{kasperksy}. 
...
For this, we execute, end-to-end, the variants that fully evade VirusTotal when generated with WASM-MUTATE in controlled and uncontrolled diversification configurations.
...
We highlight in bold text the values for which the uncontrolled diversification or controlled diversification setups best, the lower, the better.
...
\wrule{Uncontrolled diversification to evade VirusTotal:}
We execute an uncontrolled diversification with WASM-MUTATE, setting a limit of 1000 iterations for each binary. 
...
Analyzing the data in \autoref{offensive:results:fast}, we observe that the controlled diversification setup successfully generates variants that totally evade the detection for 30 out of 33 binaries, it is thus as good as the uncontrolled setup.
The iterations needed for the controlled diversification setup are 92\% of the iterations needed on average for the uncontrolled diversification setup.
% The best cases
...
For instance, a potential attacker may be limited by budget on the number of transformations applicable to the malware binary, e.g., the number of queries to VirusTotal. 
Additionally, the performance of the resulting binary benefits from this approach.


\wrule{Uncontrolled diversification to evade MINOS: }
...
We incrementally apply stacked transformations to the original malware binary until MINOS is completely evaded or the maximum iteration limit is achieved. 
...
Consequently, this supports the employment of multiple malware oracles (meta-oracles) for the detection of cryptojacking malware in browsers, even when it is possible a Wasm-specific detection mechanism.
...
\wrule{\Wasm execution impact:} To evaluate the real-world efficacy of WASM-MUTATE in evading malware detection, we focused on six binaries that we could build and execute end-to-end, as these had all three components outlined in \autoref{fig:attack_crypto}. 
...
We assessed the variants based on the hash generation rate, providing a hint on the impact of diversification in the \Wasm variants execution.
