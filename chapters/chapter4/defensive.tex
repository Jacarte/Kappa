...
\begin{strategy}[Disrupting timers]
    ...
    We create variants that offer a similar approach.
...
\begin{strategy}[Managed memory impact]
    The success in diminishing Spectre attacks is explained by the fact that \tool synthesizes variants that effectively alter memory access patterns. 
...
\begin{tcolorbox}[title=Reflection,boxrule=1pt,arc=.2em,boxsep=1.0mm]
    ...
    For instance, CROW and MEWE do not directly modify memory accesses. 
    ...
    This change is the first inherent property of diversification processes and significantly affects memory accesses, despite not explicitly targeting the \Wasm binary's memory model. 
