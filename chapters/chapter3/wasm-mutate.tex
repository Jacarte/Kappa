...
In the LHS part of the rewriting rule, the loop showcases the first-order break, signaling that its activation prompts continuous loop iterations. 
...
\wrule{\texttt{useglobal:}} this operator is used in a rewriting rule that substitutes its operand with the setting and retrieval actions involving a newly created global variable. In the following listing, we illustrate such rewriting rule.
...
This means that, when converting Wasm to the intermediate representation of WASM-MUTATE they are not used.
...
The rewriting rule is designed to express the equivalence of conducting an \texttt{or} operation where both operands are identical.
...
The process begins with the incorporation of the solitary program instruction, \texttt{i64.const 0}, as an e-node \step{1}.
