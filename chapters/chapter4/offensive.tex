...
After running the wasmbench dataset through VirusTotal and MINOS, we identify 33 binaries: 1)flagged as malicious by at least one VirusTotal vendor and 2) also detected by MINOS.
...
\begin{strategy}[Feedback-guided diversification to evade VirusTotal]
    ...
    Analyzing the data in \autoref{offensive:results:fast}, we observe that the feedback-guided diversification setup successfully generates variants that totally evade the detection for 30 out of 33 binaries, it is thus as good as the stochastic setup.
    The iterations needed for the feedback-guided diversification setup are 92\% of the iterations needed on average for the stochastic diversification setup.
    % The best cases
    ...
    For example, \texttt{f0b24409} needs 11 oracle calls with feedback-guided diversification setup to fully evade VirusTotal, while for the stochastic one, it needs 127 oracles calls.
    For those 21 binaries, it needs only 40\% of the calls the feedback-guided diversification setup needs.
    The feedback-guided diversification setup, having fewer stacked transformations overall, demonstrates larger efficiency than the stochastic diversification setup. 
...
\begin{strategy}[Stochastic diversification to evade MINOS]
    ...
    The median iteration count needed to evade VirusTotal is 218 for the stochastic diversification setup, and 131 for the feedback-guided diversification setup; however, only 38 iterations are needed for MINOS.
...
\begin{strategy}[\Wasm execution impact]
...
\begin{tcolorbox}[title=Contribution paper,boxrule=1pt,arc=.2em,boxsep=1.0mm]
    WASM-MUTATE generates variants of WebAssembly cryptojacking that successfully evade malware detection.
