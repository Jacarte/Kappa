...
\todo{Redo, key concepts, seminal works, end with a paragraph putting into Wasm context.}
...
\msubsection{Software Diversification Asssessment}
...
First, as previously discussed, Software Diversification is a preemptive strategy, and in most instances, specific attacks might not be replicated. 
Second, it necessitates the deployment and testing of numerous variants. 
Hence, a combination of static and dynamic metrics is required for the creation and assessment of software diversification.

\begin{strategy}[Static comparison of variants]
    \label{static_based}
    Static metrics are utilized to evaluate and identify the diversity of programs without necessitating execution. 
    ...
    For example, the most commonly used metrics for measuring static diversity in generated variants involve edit distances. 
    At the low-level of bytecode instructions, these metrics include the Jaccard distance between n-grams \cite{ngrams}, the Levenshtein distance \cite{Lev}, and global alignments \cite{CROW}. 
    At the high-level of source code, these metrics often rely on Abstract Syntax Tree (AST) diffing, such as GUMtree-based distances \cite{gumtree}. 
    While these metrics have both strengths and weaknesses, their effectiveness varies. 
    This variability is contingent upon the level of granularity and the point of application of diversification, whether applied at coarse or fine-grained levels.
    For example, n-grams of static bytecode instructions effectively operate at the fine-grained level. 
    Conversely, Gumtree and AST diffing methods excel in assessing high-level diversification. 
    This discrepancy primarily stems from the fact that low-level representations are usually less abstract.
\end{strategy}
...
\begin{strategy}[Dynamic comparison of variants]
    ...
    Therefore, dynamic metrics are essential to assess the diversity of variants during execution. 
    Many previously mentioned static metrics can be applied to execution traces of program variants. 
    ...
    Cabrera \etal used a global alignments approach to measure the diversity of JavaScript bytecode traces in the Chrome browser \cite{STRAC}. 
    Fang \etal \cite{8482113} also proposed mitigating JavaScript obfuscation techniques used in malicious code by analyzing dynamic information recorded in V8 bytecode traces. 
    While these metrics are primarily used to cluster similar behaviors, they can also be utilized to evaluate the diversity of program variants. 
    In other words, the larger the difference between two behaviors, the more diverse the variants are during runtime.
\end{strategy}
...
For example, VeriWasm quantifies attack-based patterns, postulating that a \Wasm binary is more secure with a lower pattern count \cite{veriwasm}.
...
Regarding the dynamic aspect, Wang \etal's study \cite{SEISMIC} profiles \Wasm instructions during runtime to identify malicious behavior.


%In our work on Software Diversification for \Wasm, we utilized several metrics. 
%For example, we employed global alignment metrics for \Wasm binaries to assess the static diversity of generated variants \cite{STRAC,CROW}. 
%Moreover, we supplement static analysis with trace comparisons during execution. 
%Specifically, we traced the stack-based operations of \Wasm binaries to evaluate the dynamic diversity of generated variants \cite{STRAC, CROW}. 
%Moreover, as JIT compilers compile \Wasm binaries to machine code, we compared the machine code traces of generated variants during execution \cite{wasmmutate}.
