...
First, as previously discussed, Software Diversification is a preemptive strategy. 
Second, it needs the deployment and testing of numerous variants. 
...
\begin{strategy}[Dynamic comparison of variants]
    ...
    Notice that dynamic assessment has its limitations; it is often expensive to execute the program or sometimes impossible, for example, when user interaction is required which complicates the execution process.
    Hence, in practice, dynamic assessment of variants is typically conducted during production.
    Contrarily, malware programs usually do not require user interaction for execution, making it easier to evaluate their diversity in controlled environments prior to real deployment.
\end{strategy}
