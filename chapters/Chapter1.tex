...
Third-party compilers produce \Wasm binaries, which may include optimizations, as is the case of LLVM. 
The Wasm language abstracts its Instruction Set Architecture, making it akin to machine code instructions but independent of CPU architectures \cite{wasm_spec}. 
...
Studies have highlighted the benefits of using WebAssembly as an intermediary layer, such as improved startup times and enhanced memory usage, compared to containerization and virtualization \cite{pMendkiServerless, 1244493Jacobsson}. 
In light of these findings, the Bytecode Alliance launched the WebAssembly System Interface (WASI) in 2019 \cite{bytecodealliance, WASI}. 
WASI made possible the execution of \Wasm with a POSIX system interface protocol, thereby enabling Wasm's direct execution in the operating system.
While \Wasm iss a strong promise to solve decades of persisting issues in the web environment, it also introduces new challenges.
...
In browsers, Rokicki et al. revealed the risk for port contention side-channel attacks on \Wasm binaries in browsers \cite{10.1145/3488932.3517411}. 
In standalone deployments, Genkin et al. demonstrated the potential for data extraction via cache timing-side channels in \Wasm \cite{Genkin2018DrivebyKC}. 
