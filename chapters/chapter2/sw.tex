...
Rewriting strategies typically take the form of tuples: \texttt{instr1 => (instr2, instr3, ...)}, where \texttt{instr} represents the original code and \texttt{(instr2, instr3, ...)} denotes the functionally equivalent replacement.
...
In the following, we group the major strategies used to artificially generate software variants and their current state concerning \Wasm.
...
For instance, Cleemput et al. \cite{Cleemput2012} and Homescu et al.~\cite{homescu2013profile} inject NOP instructions to produce statically varied versions. 
