...
\todo{State that the following is a summary of the works of Forrest, Cohen, Jackson and Baudry etal.}
...
\begin{checking}[AI-driven checking]
    \label{ai_drove}
...
\msubsection{Software Diversitication Asssesment}

\todo{How to assess diversity? Static diffs, Dynamic Diffs, ngrams on binaries, ngrams on traces, etc.}

\todo{Focus on Size of the Space versus Complexity by Cohen. Large space does not mean a difficult attack. So you need to measure this. For example, same machine code when generating is a problem because it is easy to remove the diversification. Focus then on preservation metric. }
