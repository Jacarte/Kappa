...
As outlined in Baudry \etal's survey \cite{natural_diversity}, Software Diversitication falls into five usage categories: reusability \cite{pohl2005software},performance \cite{10.1145/2025113.2025133}, fault tolerance \cite{1659219}, software testing \cite{Chen2010AdaptiveRT}, and security \cite{cohen1993operating}. 
...
Software variants are functionally equivalent versions of an original program, created through Software Diversitication at different stages of the software lifecycle, such as the source code or machine code levels. 
%\todo{Two sentences about when: Design and implementation, Compilation and linking, post Installation, and load time.}
The diversification can be either natural \cite{natural_diversity} or atumatic \cite{offensive_div}.
...
\todo{Innovation, money, requirements.}
\todo{Interoperable as possible while offer distinct and subtle differences.}
...
\todo{We are not talking about N-variant development.}
\todo{Natural use of dependencies. Paper of Cesar about how many dependencies everyone shares.}
\todo{There is a natural force that breaks diversity. Same dependencies, etc.}
...
Artificial Software Diversitication, as proposed by Cohen and Forrest in the 1990s \cite{cohen1993operating, 595185}, gets its development through rewriting strategies. 
...
This dissertation focuses on artificial Software Diversitication, as it is more practical, systematic, and scalable than natural diversity.


\begin{strategy}[Rewriting strategy]
    \label{rewriting_strategy}
    The creation of artificial Software Diversitication commences with rewriting rules.
...
Measuring Software Diversitication presents a significant challenge. 
...
\begin{strategy}[Dynamic comparison of variants]
    ...
    %Koopman and DeVale compare 13 POSIX operating system implementations \cite{comparing_unix} through observing system call traces, demonstrating that by combining them, the failure rate diminishes.
...
By making software less predictable, defensive Software Diversitication aims to proactively deter attacks, acting as a complementary strategy to other, more reactive, security measures. 
...
The process of officially adopting a new defensive measure is inherently slow, making Software Diversitication a potentially valuable preemptive strategy. 
...
\todo{Second point needs to be stronger. Motivate with another sentence. Make it an explicit contribution. This thesis sis the first to investigate Software Diversity in depth for the emerging Wasm ecosystem. Same for third, why it needs to be explored.}
